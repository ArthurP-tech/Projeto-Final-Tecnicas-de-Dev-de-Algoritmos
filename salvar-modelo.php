    ...
    switch ($_REQUEST['acao']) {
        case 'cadastrar':
            ...
            $sql = "INSERT INTO modelo 
                (nome_modelo, ano_modelo, cor_modelo, marca_id_marca)
                VALUES ('{$nome}', '{$ano}', '{$cor}', '{$id_marca}')";
        ...
        case 'editar':
            ...
            $ano = $_POST['ano_modelo'];
            $cor = $_POST['cor_modelo'];
            $sql = "UPDATE modelo SET nome_modelo='{$nome}', ano_modelo='{$ano}', cor_modelo='{$cor}' WHERE id_modelo=".$_REQUEST['id_modelo'];
            $res = $conn->query($sql);
            if($res == true){
